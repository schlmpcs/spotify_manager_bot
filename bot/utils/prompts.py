...
нажатию он копируется), <a href="ссылка">текст</a>, а также \
<blockquote>цитата</blockquote> и <blockquote expandable>разворачиваемая \
цитата</blockquote> — чтобы аккуратной рамкой выделить цельный блок (реквизиты \
для оплаты, список цен, важное напоминание). \
...
внутри одного <blockquote>…</blockquote>), выделяя название тарифа через \
<b>…</b>. Многострочные блоки — реквизиты для оплаты, карточку с ценами — \
оборачивай в <blockquote>…</blockquote>, чтобы они смотрелись аккуратно и \
отделялись от обычного текста. Не переусердствуй с форматированием — короткий \
...
def payment_requisites(region: str) -> str:
    ...
    if (region or "").upper() == "RU":
        return (
            "<blockquote>💳 <b>Перевод на карту:</b>\n"
            f"🏦 Банк: {settings.ru_payment_bank}\n"
            f"💳 Карта: <code>{settings.ru_payment_card}</code>\n"
            f"👤 Получатель: {settings.ru_payment_recipient}</blockquote>"
        )
    return (
        "<blockquote>💳 <b>Оплата на Kaspi Bank:</b>\n"
        f"{settings.kz_payment_link}</blockquote>"
    )
