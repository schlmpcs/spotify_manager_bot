...
│   │   └── owner.py         # Owner commands (/start /status /style /auto /client /nudge) + draft approvals
│   ├── services/
│   │   ├── llm.py           # Async OpenAI-compatible client — chat(messages) -> str | None (model fallback chain)
│   │   ├── admin_assistant.py # Owner DM assistant for client DB questions
...
All customer-facing sends go through `dispatch.deliver()`, which calls `bot.send_message(chat_id, text, business_connection_id=conn_id)` (and a typing action first). **A connected business bot may only send to a chat that had an incoming message from the customer in the last 24h** (that is all the `can_reply` right grants; outside the window Telegram returns `BUSINESS_PEER_USAGE_MISSING`). When delivery fails, `send_or_approve` falls back to a **manual-send card**: it drops the "Отправить" button (a re-send would hit the same limit), translates the raw error into a plain-language reason (`_manual_reason`), shows the text in a tap-to-copy `<pre>` block, and (when the customer's @username is known) offers a "✍️ Написать" button that opens the chat with the message **pre-filled in the input box** via `https://t.me/<username>?text=…` (`_chat_url` → `manual_send_kb`), so the manager just opens it and taps Send from their own account — which has no 24h limit. The pre-filled / copied text is run through `_plain()` to strip the Telegram-HTML tags (`<b>`, `<code>`) since a hand-sent message isn't HTML-parsed; the prefill is dropped if the resulting URL would exceed ~2000 chars (Telegram can reject over-long button URLs). Prefill needs a **valid** public username (`_valid_username`: starts with a letter, 5–32 chars) — the payment DB sometimes stores `str(user_id)` or a short value in `users.username`, and a `t.me/<that>` link resolves to "user doesn't exist", so those customers get no button and just the copy block. The id is shown as a tap-to-copy `<code>` (not a `tg://user?id=` link, which could fail to send for a customer the manager-bot has never seen and would sink the whole card).
...
| `/client <id\|@username\|name>` | Look up client/payment status from the PostgreSQL payment DB |
...
- **Rich formatting:** messages use Telegram HTML. Payment requisites are wrapped in `<blockquote>`. The **full price list is deterministic, not LLM-typed**: the model can't reliably reproduce exact HTML/whitespace (it flattens blockquotes and column alignment), so it emits `prompts.PRICE_TABLE_MARKER` and `business.py` swaps it for `prompts.price_table()` — a perfectly-aligned monospace `<pre>` table built in code (Telegram's native June-2026 tables aren't exposed via bot HTML, so `<pre>` is the reliable aligned grid). Single-price questions are still answered conversationally by the model.
- **Custom emoji:** `dispatch.deliver` runs every outgoing text through `emoji.decorate` (`bot/utils/emoji.py`), which upgrades any glyph mapped in `CUSTOM_EMOJI_IDS` to a Premium **custom (animated) emoji** (`<tg-emoji emoji-id="…">`). Empty map = off (default). **Known limitation:** Telegram appears to ignore bot custom-emoji entities on messages sent **on behalf of a business account** (the `business_connection_id` path), so customers see the plain fallback glyph even with Premium — same class of restriction as custom keyboards not working for business messages. The `/emojiid` self-test renders the emoji in a *direct* bot DM to confirm the IDs/Premium are valid in isolation. If Telegram outright rejects the entity, `deliver` resends un-decorated and `emoji.note_failure()` disables the upgrade for the run. Manual-send cards strip all tags via `_plain`.
